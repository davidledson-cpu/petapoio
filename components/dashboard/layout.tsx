...
export function DashboardLayout({ children, userRole, userName, userAvatar }: DashboardLayoutProps) {
  const nav = userRole === 'admin' ? adminNav : userRole === 'patient' ? patientNav : professionalNav
      ...
      <div className="px-5 py-4 border-b border-petblue-50">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-gray-400">{userRole === 'admin' ? 'Administrador' : userRole === 'patient' ? 'Paciente' : 'Profissional'}</div>
          </div>
        </div>
      </div>
