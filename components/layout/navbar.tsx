        ...
        <ul className="hidden md:flex items-center gap-8">
          {navLinks.map(link => (
            <li key={link.href}>
              <Link
                href={link.href}
                className="text-sm font-semibold text-gray-600 hover:text-petblue-500 transition-colors"
              >
                {link.label}
              </Link>
            </li>
        ...
        <div className="hidden md:flex items-center gap-3">
          {user ? (
            <>
              <Link
                href={
                  user.role === 'professional' ? '/dashboard/profissional'
            ...
            {user ? (
              <>
                <Link
                  ...
                  className="block py-2 text-sm font-bold text-petblue-600"
                >
                  Meu Dashboard
                </Link>
                <button onClick={handleSignOut} className="block py-2 text-sm font-semibold text-gray-500">
                  Sair
                </button>
              </>
            ) : (
