...
export function ProfissionaisClient({ patientName, patientEmail }: ProfissionaisClientProps) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">
          Psicólogos especializados em luto animal, prontos para apoiar você
        </p>
      </div>
      ...
      <div className="grid gap-4">
        {PROFESSIONALS.map((pro) => (
          <div
            key={pro.id}
            className="bg-white rounded-2xl border border-petblue-100 p-5 shadow-sm hover:shadow-md transition-shadow"
          >
            <div className="flex items-start justify-between gap-4">
              <div className="flex items-start gap-4">
                <div className="w-14 h-14 rounded-xl bg-petblue-50 flex items-center justify-center text-2xl flex-shrink-0">
                  {pro.avatar}
                </div>
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    <h2 className="font-bold text-gray-800 text-base">{pro.name}</h2>
                    <span className="text-xs bg-petblue-50 text-petblue-600 px-2 py-0.5 rounded-full font-medium">
                      {pro.crp}
                    </span>
                    {!pro.available && (
                  ...
                  <p className="text-sm text-petblue-500 font-medium mt-0.5">{pro.specialty}</p>
              ...
              <div className="flex-shrink-0">
                <button
                  ...
                  className={`px-5 py-2.5 rounded-xl text-sm font-bold transition-all ${
                    pro.available
                      ? 'bg-petblue-400 hover:bg-petblue-500 text-white shadow-sm hover:shadow'
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex items-center justify-between p-5 border-b border-gray-100">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-petblue-50 flex items-center justify-center text-xl">
                  {selectedProfessional.avatar}
                </div>
                <div>
                  <h3 className="font-bold text-gray-800">{selectedProfessional.name}</h3>
                  <p className="text-xs text-petblue-500">{selectedProfessional.specialty}</p>
                </div>
              </div>
              <button
                onClick={closeModal}
            ...
            {success ? (
              ...
              <div className="p-8 text-center">
                ...
                <div className="bg-petblue-50 rounded-xl p-4 text-left mb-6">
                  <div className="flex items-center gap-2 text-sm text-petblue-700 mb-1">
                  ...
                  <div className="flex items-center gap-2 text-sm text-petblue-700">
                    <Clock className="w-4 h-4" />
                ...
                <button
                  onClick={closeModal}
                  className="w-full py-3 bg-petblue-400 hover:bg-petblue-500 text-white rounded-xl font-bold transition-colors"
                >
                  Fechar
                </button>
              </div>
              ...
              <div className="p-5">
                ...
                <div className="mb-5">
                  <div className="flex items-center gap-2 mb-3">
                    <Calendar className="w-4 h-4 text-petblue-500" />
                      ...
                      <button
                        ...
                        className={`flex-shrink-0 px-3 py-2.5 rounded-lg text-xs font-medium border transition-all ${
                          selectedDate?.toDateString() === day.toDateString()
                            ? 'bg-petblue-400 text-white border-petblue-400'
                            : 'bg-white text-gray-700 border-gray-200 hover:border-petblue-300'
                        }`}
                      >
                        {formatDay(day)}
                      </button>
                ...
                {selectedDate && (
                  <div className="mb-5">
                    <div className="flex items-center gap-2 mb-3">
                      <Clock className="w-4 h-4 text-petblue-500" />
                ...
                {selectedDate && selectedTime && (
                  <div className="mb-5">
                    <div className="flex items-center gap-2 mb-3">
                      <MessageSquare className="w-4 h-4 text-petblue-500" />
                    ...
                    <textarea
                      ...
                      rows={3}
                      className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-petblue-300 focus:border-transparent resize-none"
                    />
                  </div>
                )}
                ...
                {/* Summary */}
                {selectedDate && selectedTime && (
                  <div className="bg-petblue-50 rounded-xl p-4 mb-5">
                    <p className="text-xs text-petblue-500 font-bold uppercase tracking-wide mb-2">
                  ...
                  <button
                    ...
                    className={`flex-1 py-3 rounded-xl font-bold text-sm transition-all ${
                      selectedDate && selectedTime && !loading
                        ? 'bg-petblue-400 hover:bg-petblue-500 text-white shadow-sm'
                        : 'bg-gray-100 text-gray-400 cursor-not-allowed'
                    }`}
                  >
